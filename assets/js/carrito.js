...
function agregarDeseo(idProducto) {
  ...
  alertaPersonalizada("Producto agregado a la lista de deseos", "success");
  cantidadDeseo();
}
...
function agregarCarrito(idProducto, cantidad, accion = false) {
  ...
  // Verificar si es necesario eliminar de la lista de deseos
  if (accion) {
  ...
  http.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      ...
      res.productos.forEach((producto) => {
        html += `<tr>
                    <td>
                    <img class="img-thumbnail rounded-circle carrito-imagen" src="${
                    ...
                    <button class="btn btn-danger btnDeletecart" type="button" prod="${
                      producto.id
                    }"><i class="fas fa-time-circle custom-font">Eliminar</i></button>
                    </td>
                </tr>`;
      });
