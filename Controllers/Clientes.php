...
class Clientes extends Controller
{
    ...
    public function enviarCorreo()
    {
        if (isset($_POST['correo']) && isset($_POST['token'])) {
            ...
            try {
                ...
                //Recipients
                $mail->setFrom('jorgeavera@live.com.ar', TITLE);
                ...
                $mail->AltBody = 'Gracias por elegir TIENDAISP6';
    ...
    public function verificarCorreo($token)
    {
        ...
        if (!empty($verificar)) {
            ...
            header('Location: ' . BASE_URL . '?verificacion=exitosa');
        }
    }
    ...
    public function registrarPedido()
    {
        ...
        if (is_array($pedidos) && is_array($productos)) {
            ...
            $_SESSION['datos_transaccion'] = array(
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            $data[$i]['calificacion'] = '<ul class="list-unstyled d-flex justify-content-between">
                <li class="calificacion">
                    <i class="' . $uno . ' fas fa-star" onclick="agregarCalificacion(' . $data[$i]['id_producto'] . ', 1)"></i>
                    <i class="' . $dos . ' fas fa-star" onclick="agregarCalificacion(' . $data[$i]['id_producto'] . ', 2)"></i>
                    <i class="' . $tres . ' fas fa-star" onclick="agregarCalificacion(' . $data[$i]['id_producto'] . ', 3)"></i>
                    <i class="' . $cuatro . ' fas fa-star" onclick="agregarCalificacion(' . $data[$i]['id_producto'] . ', 4)"></i>
                    <i class="' . $cinco . ' fas fa-star" onclick="agregarCalificacion(' . $data[$i]['id_producto'] . ', 5)"></i>
                </li>
            </ul>';
        }
        echo json_encode($data);
